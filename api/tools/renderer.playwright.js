// Local renderer using Playwright. Returns the SAME payload shape as puppeteer version.
...
module.exports = async function rendererPlaywright(params) {
    // ---- Params
    const url = String(params.url || "").trim();
    let viewport = params.viewport ?? params.form_factor ?? "desktop";
    viewport = String(viewport).toLowerCase() === "mobile" ? "mobile" : "desktop";
    const timeout_ms = Number(params.timeout_ms ?? 30000);

    const BODY_LIMIT_KB = clampKb(params.html_body_kb, clampKb(process.env.HTML_BODY_KB, 8));
    const HEAD_LIMIT_KB = clampKb(params.head_limit_kb, clampKb(process.env.HTML_HEAD_KB, 8));
    const STRIP_SCRIPTS = toBool(params.strip_scripts, true);
    const RETURN_EXTRACTED = toBool(params.return_extracted, true);
    ...
    try {
        ...
        try { await page.waitForLoadState("networkidle", { timeout: Math.min(timeout_ms, 15000) }); } catch { }
        ...
        if (RETURN_EXTRACTED) {
            extracted = await page.evaluate(() => {
                ...
                return {
                    ...
                    og, twitter: tw, h1: h1 ? h1.trim() : null, h2, intro_text: intro_words, images
                };
            });
        }
    } catch (e) {
        return {
            ok: true, rendered: false, final_url: finalURL, user_agent: userAgent,
            error: "render_failed", details: String(e?.message || e)
        };
    } finally {
    ...
    let headBlock = headMatch ? stripHeavy(headMatch[0], STRIP_SCRIPTS) : "";
    ...
    if (bodyOpenMatch) {
        ...
        if (startIdx >= bodyOpenMatch[0].length) {
            bodySlice = html.slice(startIdx, startIdx + BODY_LIMIT_BYTES);
        }
    }
    ...
    if (bodyOpenMatch) slimHtml += bodyOpenMatch[0] + bodySlice + `\n<!-- [body truncated… kept ~${BODY_LIMIT_KB}KB] -->`;
