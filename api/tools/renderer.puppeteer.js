...
// NOTE: This renderer follows the contract: (params) -> payload object
module.exports = async function renderWithPuppeteer(p = {}) {
    ...
    try {
        ...
        // Null-safe navigation (do NOT read nav.status() without a guard)
        let nav = null;
        try {
        ...
        try {
            await page.waitForNetworkIdle({ timeout: Math.min(timeout_ms, 15000) });
        } catch (_) { /* swallow */ }
    ...
    // Build a tiny, safe HTML excerpt (head + small body; scripts/styles removed)
